...
public class ServletParameters extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String secreCode = getServletConfig().getInitParameter("secretCode");
		String url = getServletContext().getInitParameter("dbURL");
		response.setContentType("text/html");
		PrintWriter out = response.getWriter();
		String outString = String.format("Secret Code: %s\tURL: %s%n", secreCode, url);
		out.println("<html><body><h1>"+outString+"</h1></body></html>");
	}
